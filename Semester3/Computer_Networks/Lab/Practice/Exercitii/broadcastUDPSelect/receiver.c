...
int main() {
    ...
    while (1) {
        // Clear the set and add the socket to the set
        FD_ZERO(&readfds);
        FD_SET(sockfd, &readfds);

        // Wait for activity on the socket (blocking)
        ...
        // If the socket is ready to be read from
        if (FD_ISSET(sockfd, &readfds)) {
            ...
            // Null-terminate the buffer to avoid any leftover characters
            ...
            // Do not broadcast the "client connected" message back to the client
