...
bool rel4(TKey c1, TKey c2) { return c1 % 1111 <= c2 % 1111; }

bool rel5(TKey c1, TKey c2) { return c1 % 1111 >= c2 % 1111; }
...
void testRelation(Relation r) {
  ...
  while (smit.valid()) {
    ...
    assert(current == smit.getCurrent().second ||
           current == smit.getCurrent().second / 2);
...
void testRemoveSearch(Relation r) {
  ...
  for (int c = min; c <= max; c++) {
    ...
    if (c % 2 == 0) assert(smm.remove(c, c) == true);
    testIteratorSteps(smm);
  }
