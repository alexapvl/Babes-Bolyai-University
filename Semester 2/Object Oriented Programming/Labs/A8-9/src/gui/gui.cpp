...
GUI::GUI(Service& service) : service(service) {
  ...
  // Admin Tab
  QObject::connect(moviesListWidget, &QListWidget::itemPressed, this, &GUI::adminItemSelectedHandler);
...
void GUI::buildGUI() {
  ...
  adminLeftLayout->addWidget(moviesListWidget);
...
void GUI::populateMovies() {
  ...
  for (auto movie : service.getMovies()) {
    QListWidgetItem* item = new QListWidgetItem{QString::fromStdString(movie.getTitle() + " | " + movie.getGenre() + " | " + std::to_string(movie.getYearOfRelease()) + " | " + std::to_string(movie.getNumberOfLikes()) + " likes")};
    QFont font{"Courier New", 15, 5};
    item->setFont(font);
...
void GUI::adminItemSelectedHandler() {
  int row = moviesListWidget->currentRow();
...
void GUI::userRateMovieWatchListButtonHandler() {
  ...
  // make a pop-up window which asks the user if they liked or not the movie(yes/no buttons)
  // if they liked the movie, increment the number of likes for that movie and delete it from the watch list
  // if they didn't like the movie, delete it from the watch list
