...
// Constructor
Service::Service() {
  this->repository = new FileRepository("../movies.txt", "../watchlist.csv", repoType::CSV);
...
bool Service::addMovieToWatchList(int index) {
  ...
  Returns true if the movie was added, false otherwise
  */
  if (this->repository->validatePositionWatchList(this->repository->returnPositionWatchList(index)))
    return false;
...
int Service::returnPosition(Movie& movie) {
  ...
  */
  return this->repository->returnPosition(movie);
}
