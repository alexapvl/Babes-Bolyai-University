...
segment data use32 class=data
    ...
    e dd 1000
    x dq 4500
    ;final result should be 6100 on a quad
...
start:
    ...
    mov AL, [a]
    mov AH, 0; unsigned conversion from AL -> AX | now we have AX = a/b
    div byte[b]; unsigned division | AH = AX % b and AL = AX / b | we will only need AL in this case(the intiger part of the division)
    mov AH, 0
    push word 0
    push AX
    pop EAX
    mov EDX, 0; EDX:EAX = a/b = 2
   ...
   mov AL, [d]
   mov AH, 0; unsigned conversion from AL -> AX | now AX = d = 100
   mul word[c]; DX:AX = c*d = 600 | we need to put DX:AX in EDX:EAX
   push DX
   push AX
   pop EAX
   mov EDX, 0; now we have c*d = 600 in EDX:EAX
   ...
   mov EAX, 0
   mov EDX, 0; reset the values of EAX and EDX for future operations and conversions
   add AL, [b]
   div word[c]; now we will have DX = b%c and AX = b/c | We will only use AX, the intiger part of the division!!!
   mov DX, 0; remove the remainder from the lower part of EDX to make the subtraction
